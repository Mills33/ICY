...
baseFounderPlot <- function(chosen_data){
  p <- ggplot(chosen_data, aes(x = Founder, y = Percent_contribution)) +
    ...
    theme(
      plot.title = element_text(margin = margin(15, 0, 30, 0), hjust = 0.5, size =20, face ="bold"),
...
shinyServer(
  function(input, output, session) {
    ...
    # and updates the options in the filter and prior release dropdown.
    ...
    chosendata <- reactive({
      ...
      if (is.null(input$id) & is.null(input$location) & (input$age =='500')) {
 ...
 #add age, location
   ...
   MK is the mean kinship coefficient which is a measure of, on average, how related that individual is to all the others in the file provided; Fe stands for the founder equivalents this is a measure of theoretical genetic diversity that takes in to account the number of founders genomes that are represented in the individual and how evenly those genomes are represented.",
